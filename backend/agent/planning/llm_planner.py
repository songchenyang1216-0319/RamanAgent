...
class LLMPlanner:
    ...
    def __init__(self, *, use_external_model: bool | None = None) -> None:
    ...
    def plan(self, normalized: NormalizedMessage, catalog: ToolCatalog) -> PlannerOutput:
        if self.use_external_model:
            try:
                ...
                # Let the deterministic planner provide a safe local plan. The
                # orchestrator still treats malformed/invalid plans as fallback.
                pass
    ...
    def _plan_with_model(self, normalized: NormalizedMessage, catalog: ToolCatalog) -> PlannerOutput:
        ...
        return PlannerOutput(plan=LLMPlan.from_dict(payload), raw=raw, source="external_llm")
